    ...
    const load = () => {
        ...
        igApi.getStatus(companyId)
            .then((s: IgStatusResponse) => {
                setStatus(s);
                if (s.connected) {
                    return igApi.getOverview(companyId, startDate, endDate).then(d => setData(d));
                }
            })
            ...
            {data && !data.errorMessage && (
                <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <MetricCard label="Takipçi Kazanımı" value={"+" + fmtNum(data.followersGained)} icon={UserPlus} color="text-emerald-400" bgColor="bg-emerald-500/10" sub={data.followersLost > 0 ? "-" + fmtNum(data.followersLost) + " kayıp" : undefined} />
