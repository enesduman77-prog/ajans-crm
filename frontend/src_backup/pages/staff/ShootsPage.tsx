                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {shoots.map((s, i) => (
                        <motion.div key={s.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                            ...
                            className="bg-[#111113] border border-white/[0.06] rounded-2xl p-6 group hover:border-violet-500/20 transition-all cursor-pointer">
                                    ...
                                    <div className="border-t border-white/[0.06] pt-5">
                                        ...
                                        <div className="space-y-2">
                                            {selected.equipment.map(eq => (
                                                <div key={eq.id} className="bg-[#111113] rounded-xl p-3 border border-white/[0.04] flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                    ...
                                    <div className="border-t border-white/[0.06] pt-5">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {selected.participants.map(p => (
                                                <span key={p.userId} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#111113] border border-white/[0.04] rounded-lg text-xs text-zinc-300">
                                ...
                                {selected.status === 'PLANNED' && (
                                    <div className="border-t border-white/[0.06] pt-5 flex gap-2">
                                        <button onClick={() => statusMutation.mutate({ id: selected.id, status: 'COMPLETED' })}
                                            className="flex items-center gap-1.5 px-4 py-2 bg-emerald-500/10 text-emerald-400 rounded-xl text-xs font-medium hover:bg-emerald-500/20 transition-colors">
                                    ...
                                    <div key={idx} className="bg-[#18181b]/50 rounded-xl border border-white/[0.06] p-3 space-y-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input type="number" min={1} value={eq.quantity} placeholder="Adet"
                                                ...
                                                className="w-16 bg-[#111113] border border-white/[0.08] rounded-lg px-2 py-1 text-sm text-white text-center" />
