...
const statusMap: Record<string, { label: string; color: string; bg: string }> = {
    ...
    COMPLETED: { label: 'Tamamlandı', color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
                    ...
                    {meetings.map(m => {
                        ...
                        return (
                            <motion.div key={m.id}
                                ...
                                className={`bg-[#111113] border rounded-2xl overflow-hidden transition-all hover:border-white/[0.12] cursor-pointer
                                                ...
                                                <div>
                                                    ...
                                                    {m.status === 'COMPLETED' && (
                                                        <p className="text-[10px] text-zinc-600 mt-1.5">
                                                            <CheckCircle2 className="w-3 h-3 inline text-emerald-500" /> Not yazıldı &nbsp;
            ...
            <AnimatePresence>
                {noteModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div
                            initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
                            className="bg-[#111113] border border-white/[0.08] rounded-2xl w-full max-w-md shadow-2xl"
                            onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
